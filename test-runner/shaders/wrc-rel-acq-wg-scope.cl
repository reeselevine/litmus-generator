...
__kernel void litmus_test (
  ...
  if(shuffled_workgroup < stress_params[9]) {
    ...
    // Thread 0
    atomic_store_explicit(&test_locations[x_0], 1, memory_order_release, memory_scope_work_group);

    // Thread 1
    uint r0 = atomic_load_explicit(&test_locations[x_1], memory_order_acquire, memory_scope_work_group);
    atomic_store_explicit(&test_locations[y_1], r0, memory_order_release);
