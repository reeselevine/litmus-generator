  ...
  if(shuffled_workgroup < stress_params[9]) {
    ...
    if (id_0_final != id_1_final) {
      if (id_1_first_half) { // one observer thread reads x then y
        ...
        uint r1 = atomic_load_explicit(&test_locations[y_1 + r0/2], memory_order_relaxed);
        ...
        uint r2 = atomic_load_explicit(&test_locations[y_1], memory_order_relaxed);
        uint r3 = atomic_load_explicit(&test_locations[x_1 + r2/2], memory_order_relaxed);
