...
__kernel void litmus_test (
  ...
  if(shuffled_workgroup < stress_params[9]) {
    ...
    uint id_1 = shuffled_workgroup * get_local_size(0) + permute_id(get_local_id(0), stress_params[7], get_local_size(0));
    uint new_workgroup = stripe_workgroup(shuffled_workgroup, get_local_id(0), stress_params[9]);
    uint id_2 = new_workgroup * get_local_size(0) + permute_id(get_local_id(0), stress_params[7], get_local_size(0));
    uint x_0 = id_0 * stress_params[10] * 3;
    uint y_0 = permute_id(id_0, stress_params[8], total_ids) * stress_params[10] * 3 + stress_params[11];
    uint permute_id_1 = permute_id(id_1, stress_params[8], total_ids);
    uint y_1 = permute_id_1 * stress_params[10] * 3 + stress_params[11];
    uint z_1 = permute_id(permute_id_1, stress_params[8], total_ids) * stress_params[10] * 3 + 2 * stress_params[11];
    uint x_2 = id_2 * stress_params[10] * 3;
    uint permute_id_2 = permute_id(id_2, stress_params[8], total_ids);
    uint z_2 = permute_id(permute_id_2, stress_params[8], total_ids) * stress_params[10] * 3 + 2 * stress_params[11];
    if (stress_params[4]) {
    ...
    atomic_store_explicit(&test_locations[z_1], r0, memory_order_relaxed);

    // Thread 2
    uint r1 = atomic_load_explicit(&test_locations[z_2], memory_order_relaxed);
    uint r2 = atomic_load_explicit(&test_locations[x_2 + r1/2], memory_order_relaxed);
