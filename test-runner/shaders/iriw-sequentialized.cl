...
__kernel void litmus_test (
  ...
  if(shuffled_workgroup < stress_params[9]) {
    ...
    uint x_0 = id_0_final * stress_params[10] * 2;
    uint y_0 = permute_id(id_0_final, stress_params[8], total_ids) * stress_params[10] * 2 + stress_params[11];
    ...
    if (id_0_first_half) { // one observer thread reads x then y
      uint r0 = atomic_load_explicit(&test_locations[x_0], memory_order_relaxed);
      uint r1 = atomic_load_explicit(&test_locations[y_0], memory_order_relaxed);
      atomic_store(&read_results[id_0_final * 4 + 1], r1);
      atomic_store(&read_results[id_0_final * 4], r0);
    } else { // other observer thread reads y then x
      uint r2 = atomic_load_explicit(&test_locations[y_0], memory_order_relaxed);
      uint r3 = atomic_load_explicit(&test_locations[x_0], memory_order_relaxed);
      atomic_store(&read_results[id_0_final * 4 + 3], r3);
      atomic_store(&read_results[id_0_final * 4 + 2], r2);
    }
